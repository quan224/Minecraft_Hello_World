...
const int noiseTextureResolution = 256;     // 噪声图分辨率
...
vec4 getBloomSource(vec4 color) {
    ...
    // 发光方块
    if(id>=10088.99&&id<=10089.01) {
        bloom.rgb *= 7 * vec3(2, 1, 1);
    }
    // 火把 
    else if(id>=10089.99&&id<=10090.01) {
        if(brightness<0.5) {
            bloom.rgb = vec3(0);
        }
...
void main() {
    ...
    */
    ...
    // 不是发光方块则绘制阴影
    if((id<=10088.99||id>=10089.01) && (id<=10089.99||id>=10090.01) ) {
